    ...
    def execute_command(self, command, *args):
        if command == "/Mahim ":
            try:
                ...
                if not team_code:
                    return "No team code provided for /Mahim "

                print(
                    f"[{self.account_id}] Executing /Mahim  for team code {team_code} with name {account_name}"
                )
                sys.stdout.flush()
...
def ghost_all():
    ...
    for idx, (account_id, client) in enumerate(sorted_clients):
        ...
        result = client.execute_command("/Mahim ", teamcode, account_name)
        results[account_id] = f"Dev: ROHIT | {result} | Name: {account_name}"
...
def ghost():
    ...
    result = client.execute_command("/Mahim ", teamcode, ghost_name)
